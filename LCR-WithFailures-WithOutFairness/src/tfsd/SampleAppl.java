...
import src.tfsd.KillMe.KillMeLayer;
import src.tfsd.lcr.LCRLayer;
...
public class SampleAppl {
    ...
    private static Channel getChannel(ProcessSet set, String qos) {
        ...
        if (qos.equals("aab")) { //AllAckURB channel
            return getaabChannel(set);
        }
        if (qos.equals("lcr")) { //AllAckURB channel
            return getLCRChannel(set);
        } else {
    ...
    /**
     * Builds a new Appiw QoS in the tfsd/Saa Channel with Best Effort Broadcast
     *
    ...
    /**
     * Builds a new Appia Channel with AllAck URB
     *
     * @param processes set of processes
     * @return a new uninitialized Channel
     */
    private static Channel getaabChannel(ProcessSet processes) {
        /* Create layers and put them on a array */
        Layer[] qos = {new TcpCompleteLayer(), new BasicBroadcastLayer(),
        ...
        QoS myQoS = null;
        try {
            myQoS = new QoS("AllAck Uniform Reliable Broadcast QoS", qos);
        } catch (AppiaInvalidQoSException ex) {
            System.err.println("Invalid QoS");
            System.err.println(ex.getMessage());
            System.exit(1);
        ...
        /* Create a channel. Uses default event scheduler. */
        Channel channel = myQoS.createUnboundChannel("AllAck Uniform Reliable Broadcast Channel");
        /*
         * Application Session requires special arguments: filename and . A session
         * is created and binded to the stack. Remaining ones are created by default
         */
    ...
    private static Channel getLCRChannel(ProcessSet processes) {
        /* Create layers and put them on a array */
        Layer[] qos = {new TcpCompleteLayer(), new KillMeLayer(),new BasicBroadcastLayer(),
            new TcpBasedPFDLayer(), new LCRLayer(),
            new SampleApplLayer()
        };
        ...
        QoS myQoS = null;
        try {
            myQoS = new QoS("TotalOrder Uniform Reliable Broadcast QoS", qos);
        } catch (AppiaInvalidQoSException ex) {
            System.err.println("Invalid QoS");
            System.err.println(ex.getMessage());
            System.exit(1);
        ...
        /* Create a channel. Uses default event scheduler. */
        Channel channel = myQoS.createUnboundChannel("TotalOrder Uniform Reliable Broadcast Channel");
        /*
         * Application Session requires special arguments: filename and . A session
         * is created and binded to the stack. Remaining ones are created by default
         */
    ...
    private static void invalidArgs(String reason) {
        System.out
                ...
                + "\n\t lcr - Total Order Uniform Broadcast"
                + "\n\t aab - AllAck Uniform Reliable Broadcast");
        System.exit(1);
    }
}
