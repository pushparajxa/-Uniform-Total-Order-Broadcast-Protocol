...
//            }
		...
		private void handleClientMessage(ClientMessage msg, SocketAddress addr){
			...
			try {
                serverMsg.marshal();
                byte[] bytes = Constants.createMessageToSend(Constants.MessageType.SERVER, serverMsg.getByteArray());
		...
		private void handleServerMessage(ServerMessage smsg, SocketAddress addr){
		    try {
		        ...
		        myMsg.marshal();
		        byte[] bytes = Constants.createMessageToSend(Constants.MessageType.CLIENT, myMsg.getByteArray());
