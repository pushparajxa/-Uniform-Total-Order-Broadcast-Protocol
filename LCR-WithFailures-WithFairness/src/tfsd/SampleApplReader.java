...
public class SampleApplReader extends Thread {
    ...
    public void run() {
        /*     long time = System.currentTimeMillis();
         int len =this.processes.getSize();
         int myRank=this.processes.getSelfRank();
         while(System.currentTimeMillis() - time < ((len-1) -myRank)*1000 + 9000);*/
        try {
            Thread.sleep(10000);
        ...
        while (true) {
            ...
            try {
                ...
                SampleSendableEvent async;
                Message messages;
                int i = 0;
                while (i != 3000) {
                    //is this refreshing??
                   // if (!parentSession.readerBlock) {
                        async = new SampleSendableEvent();
                        messages = async.getMessage();
                        String msg;
                        if(i==0){
                            async.setCommand("startpfd");
                            msg="";
                        }
                        else{
                        async.setCommand("bcast");
                         msg = "test" + i;
                        }
                        ...
                        async.asyncGo(parentSession.channel, Direction.DOWN);
                        i++;
                    }
